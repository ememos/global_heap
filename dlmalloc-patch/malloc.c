...
#ifdef GLOBAL_HEAP
#define NUM_GLOBAL_HEAPS 100

static int global_heap_flag;

static struct malloc_state av_[NUM_GLOBAL_HEAPS];  /* never directly referenced */
#else
...
#ifdef GLOBAL_HEAP
#define get_malloc_state() (&(av_[global_heap_flag]))
#else
...
#define SYS_global_heap_switch __NR_global_heap_switch
#define SYS_global_heap_print_metadata __NR_global_heap_print_metadata
#define SYS_global_heap_switch2 __NR_global_heap_switch2
#define SYS_global_heap_remove __NR_global_heap_remove

int gheap_switch(int heap_id)
{
        int ret = 0;
        ...
        ret = syscall(SYS_global_heap_switch, heap_id);
        if (ret < 0) {
                errno = -ret;
                goto out;
        }

	global_heap_flag = heap_id;
...
int gheap_switch2(int heap_id)
{
        int ret = 0;
	struct malloc_state *av_src = get_malloc_state();
	char *src = (char *)av_src;
	struct malloc_state *av_dst;
	char buf[2048];
        ...
        ret = syscall(SYS_global_heap_switch2, heap_id, src, buf); 
	if (ret < 0) {
                errno = -ret;
                goto out;
	} else {
		//TODO:		
		global_heap_flag = heap_id;
		av_dst = get_malloc_state();
		memcpy((char *)av_dst, buf, sizeof(struct malloc_state));
#if 0
		printf("heapID[%d] ------------ global_heap_flag[%d]\n", heap_id, global_heap_flag);
		printf(" av_[%d].top_pad = %ld\n", global_heap_flag, av_[global_heap_flag].top_pad);
		printf(" av_[%d].mmap_threshold = %ld\n", global_heap_flag, av_[global_heap_flag].mmap_threshold);
		printf(" av_[%d].trim_threshold = %ld\n", global_heap_flag, av_[global_heap_flag].trim_threshold);
		printf(" av_[%d].top = %p\n", global_heap_flag, av_[global_heap_flag].top);
		printf(" av_[%d].pagesize = %d\n", global_heap_flag, av_[global_heap_flag].pagesize);
#endif
	}
...
int gheap_print_metadata(int heap_id)
{
        ...
        ret = syscall(SYS_global_heap_print_metadata, heap_id);
        if (ret < 0) {
                errno = -ret;
                goto out;
        }
...
int gheap_remove(int heap_id)
{
        ...
        ret = syscall(SYS_global_heap_remove, heap_id);
        if (ret < 0) {
                errno = -ret;
                goto out;
        }
